...
DOWNLOAD_TAG=$(curl -Ls $API_URL | grep "tag_name" | sed -E 's/.*"([^"]+)".*/\1/')
DOWNLOAD_URL=$(curl -Ls $API_URL | grep "browser_download_url" | grep "linux" | grep "amd64" | cut -d '"' -f 4)

curl -Ls $DOWNLOAD_URL | sudo -E tar -zxf - -C /usr/local/bin/ ghr_${DOWNLOAD_TAG}_linux_amd64/ghr --strip-components 1
